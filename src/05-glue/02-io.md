...
We call functions with side-effects, such as `addWithInput`, **impure functions**.
...
from non-I/O expressions and will also require that to *combine*
...
built-in type hidden by the Haskell language rather than a module.
...
the same **description** of a program. This helps us keep the purity and equational reasoning!
