...
At a later chapter, we will add a little fancier argument parsing
using a library, and also read whole directories and not single files.

But first, we need to learn a bit about IO in Haskell, what makes
...
how do we model interaction with the outside world? How do we do IO?

The challenge with doing IO operation in a language with a lazy evaluation strategy
...
primitive function, we then evaluate it's arguments, `readIntFromStdin`
...
Therefore, for Haskell, it was decided to only allow **pure** and expressions - ones that
...
Here are a few functions that return `IO` operations:
...
Just like our `Html` type, the IO interface provides **combinators** for composing
small `IO` operations into bigger ones. This interface also makes sure that the order
of operations is well defined!

Note that, just like with `<>` we've defined for `Html`, the combinators for `IO`
...
1. Echo
   ...
   We are reading a line from the standard input pass on the left of `>>=`,
...
2. Appending two inputs
   ...
   Note that:
     ...
     - Because of the scoping rules in Haskell, `honorific` will still be in scope for the most inner function
...
be used instead of `>>`, which only still exists to avoid breaking backwards
compatibility.
...
This combinator is useful when you want to run several IO operations one after
...
like `*>` and `>>`, `pure` is a more general version of `return` that also has the
advantage of not having a resemblance to unrelated keywords in other languages.
...
This function is useful when we want to do some computation that depends on `IO`.
...
With `fmap`, we can remove the `pure` part:
...
and is not really different from `Maybe` and `Either`. We can pass it to
functions, put it in a container, etc. Remember that it represents a description
of a program, and without being combine into `main` in some way won't actually
*do* anything.
...
do. Look back at the examples above for inspiration. Use `error` to report an error.
...
While using `>>=` to chain IO action is manageable, Haskell provides
...
- An expression of type `IO ()`, such as:
  - `putStrLn "Hello"`
...
- A binding `<variable> <- <expresion>`, such as
  ```hs
  line <- getLine
  ```
...
These construct will be translated to `<expression> *>`, `let ... in` and
`<expression> >>= \<variable>` respectively.
